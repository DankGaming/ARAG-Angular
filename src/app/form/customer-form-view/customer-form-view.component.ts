...
export class CustomerFormViewComponent implements OnInit {
	...
	submitAnswers(form: NgForm): void {
		if (this.authService.isLoggedIn()) {
			...
			modal.instance.subtitle =
				"U bent ingelogd dus er wordt geen mail verzonden";
			return;
		}
		if (
			this.form.inputs.filter(
				(input: FormInput) => input.type.name === "FILE"
			).length !== Object.keys(this.attachments).length
		) {
			const modal = this.modalService.createModal(
				AlertBoxModalComponent,
				this.modalHost
			...
			modal.instance.subtitle =
				"Controleer of u alles juist heeft ingevoerd";
			return;
		}
		...
		this.formService
			...
			.subscribe(
				...
				() => {
					...
					modal.instance.subtitle =
						"Wellicht heeft u een veld niet ingevoerd.";
				}
			);
	}
